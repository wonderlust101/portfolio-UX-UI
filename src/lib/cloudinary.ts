...
export function buildNamedTransformUrlVideo(publicId: string, namedTransform: string, ): string {
    return new CloudinaryVideo(publicId, {cloudName}).namedTransformation(name(namedTransform)).toURL();
}